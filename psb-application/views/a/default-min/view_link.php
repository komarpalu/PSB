...
<div class="content-layout"><div class="content-layout-row"><div class="layout-cell layout-item-0" style="width: 100%" >
   <?php $template = array(
      ...
      echo '<p>'.anchor('a/link/add', 'Tambah', 'class = "button"').'</p>';
      $this->table->set_heading('Tekt', 'URL', 'Aksi');
      foreach ($query as $row){
         $url = base_url().'a/link/delete/'.encode($row['link_id']).'/do';
         $data_nama = $row['link_text'];
         ...
         $attrib_edit = array('class' => 'button',
            'style' => 'width: 50px; height: 20px; line-height: 20px; padding-left: 0px; padding-right: 0px;');
         $delete = base_url().'a/link/delete/'.encode($row['link_id']);
         $edit = base_url().'a/link/edit/'.encode($row['link_id']);
         $this->table->add_row('<td style="text-align: left;">'.$row['link_text'].'</td>',
            '<td style="text-align: left;">'.anchor($row['link_url'], ellipsize($row['link_url'], 100, 0.5), 'target=new').'</td>',
