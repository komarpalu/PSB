...
<div class="content-layout"><div class="content-layout-row"><div class="layout-cell layout-item-0" style="width: 100%" >
   <?php $template = array( 'table_open'         => '<table id="tsorter" class="tablesorter" style="width: 100%;">',
      ...
      echo '<p>'.anchor('a/prov/add', 'Tambah', 'class = "button"').'</p>';
      $this->table->set_heading('Provinsi', 'Pulau', 'Keterangan', 'Aksi');
      foreach ($query as $row){
         $url = base_url().'a/prov/delete/'.encode($row['prov_id']).'/do';
         $data_nama = 'Provinsi '.$row['prov_nama'];
         ...
         $attrib_edit = array( 'class' => 'button',
             'style' => 'width: 50px; height: 20px; line-height: 20px; padding-left: 0px; padding-right: 0px;' );
         $delete = base_url().'a/prov/delete/'.encode($row['prov_id']);
         $edit = base_url().'a/prov/edit/'.encode($row['prov_id']);
         $this->table->add_row( '<td style="text-align: left;">'.$row['prov_nama'].'</td>',
            '<td style="text-align: left;">'.$row['prov_pulau'].'</td>',
            '<td style="text-align: left;">'.$row['prov_keterangan'].'</td>',
