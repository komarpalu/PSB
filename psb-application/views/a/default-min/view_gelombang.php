...
<script type="text/javascript">$(document).ready(function(){$("#tsorter").tablesorter({widgets:["zebra"],headers:{7:{sorter:!1}}})});</script>
...
<div class="content-layout"><div class="content-layout-row"><div class="layout-cell layout-item-0" style="width: 100%" >
    <?php $template = array('table_open'         => '<table id="tsorter" class="tablesorter" style="width: 100%;">',
      ...
      echo '<p>'.anchor('a/gelombang/add', 'Tambah', 'class = "button"').'</p>';
      $this->table->set_heading('TA', 'Kode', 'Gelombang', 'Tgl. Mulai', 'Tgl. Selesai', 'Jml. Pilihan', 'Keterangan', 'Aksi');
      foreach ($query as $row){
         $url = base_url().'a/gelombang/delete/'.encode($row['gel_id']).'/do';
         $data_nama = $row['gel_nama'];
         ...
         $attrib_edit = array('class' => 'button',
              'style' => 'width: 50px; height: 20px; line-height: 20px; padding-left: 0px; padding-right: 0px;');
         $delete = base_url().'a/gelombang/delete/'.encode($row['gel_id']);
         $edit = base_url().'a/gelombang/edit/'.encode($row['gel_id']);
         $this->table->add_row('<td style="text-align: center;">'.$row['gel_ta'].'</td>',
            '<td style="text-align: center;">'.$row['gel_kode'].'</td>',
            '<td style="text-align: left;">'.$row['gel_nama'].'</td>',
            '<td style="text-align: center;">'.date('d-m-Y', strtotime($row['gel_tanggal_mulai'])).'</td>',
            '<td style="text-align: center;">'.date('d-m-Y', strtotime($row['gel_tanggal_selesai'])).'</td>',
            '<td style="text-align: center;">'.$row['gel_jumlah_pilihan'].'</td>',
            '<td style="text-align: left;">'.$row['gel_keterangan'].'</td>',
...
<p><font style="font-style: italic">Gelombang yang digunakan adalah gelombang paling akhir.</font></p>
</div></div></div></div>
