...
<div class="content-layout"><div class="content-layout-row"><div class="layout-cell layout-item-0" style="width: 100%" >
   <?php $template = array(
      ...
      echo '<p>'.anchor('a/pendidikan/add', 'Tambah', 'class = "button"').'</p>';
      $this->table->set_heading('Pendidikan', 'Keterangan', 'Aksi');
      foreach ($query as $row){
         $url = base_url().'a/pendidikan/delete/'.encode($row['pend_id']).'/do';
         $data_nama = $row['pend_nama'];
         ...
         $attrib_edit = array( 'class' => 'button',
             'style' => 'width: 50px; height: 20px; line-height: 20px; padding-left: 0px; padding-right: 0px;');
         $delete = base_url().'a/pendidikan/delete/'.encode($row['pend_id']);
         $edit = base_url().'a/pendidikan/edit/'.encode($row['pend_id']);
         $this->table->add_row( '<td style="text-align: left;">'.$row['pend_nama'].'</td>',
            '<td style="text-align: left;">'.$row['pend_keterangan'].'</td>',
