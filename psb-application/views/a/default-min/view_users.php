...
<script type="text/javascript">$(document).ready(function(){$("#tsorter").tablesorter({widgets:["zebra"],headers:{6:{sorter:!1}}})});</script>
...
<div class="content-layout"><div class="content-layout-row"><div class="layout-cell layout-item-0" style="width: 100%" >
         <?php $template = array(
            ...
            if(is_admin()) echo '<p>'.anchor('a/users/add', 'Tambah', 'class = "button"').'</p>';
            $this->table->set_heading('Username', 'Email', 'Nama', 'Jabatan', 'Tgl. Input', 'Keterangan', 'Aksi');
            foreach ($query as $row){
               $url = base_url().'a/users/delete/'.encode($row['user_id']).'/do';
               $data_nama = $row['user_nama'];
               ...
               $attrib_edit = array('class' => 'button',
                  'style' => 'width: 50px; height: 20px; line-height: 20px; padding-left: 0px; padding-right: 0px;');
               $delete = base_url().'a/users/delete/'.encode($row['user_id']);
               $edit = base_url().'a/users/edit/'.encode($row['user_id']);
               if($row['user_pangkat'] == 'admin') $user_pangkat = '<b>Super Admin</b>';
               elseif ($row['user_pangkat'] == 'user') $user_pangkat = 'Admin';
               $this->table->add_row('<td style="text-align: left;">'.$row['user_username'].'</td>',
                  '<td style="text-align: left;">'.$row['user_email'].'</td>',
                  '<td style="text-align: left;">'.$row['user_nama'].'</td>',
                  '<td style="text-align: center;">'.$user_pangkat.'</td>',
                  '<td style="text-align: center;">'.$row['user_tanggal'].'</td>',
                  '<td style="text-align: left;">'.$row['user_keterangan'].'</td>',
