...
<script type="text/javascript">$(document).ready(function(){$("#tsorter").tablesorter({widgets:["zebra"],headers:{2:{sorter:false}}})})</script>
...
<div class="content-layout"><div class="content-layout-row"><div class="layout-cell layout-item-0" style="width: 100%" >
    <?php $template = array('table_open'         => '<table id="tsorter" class="tablesorter" style="width: 100%;">',
      ...
      echo '<p>'.anchor('a/agama/add', 'Tambah', 'class = "button"').'</p>';
      $this->table->set_heading('Agama', 'Keterangan', 'Aksi');
      foreach ($query as $row){
         $url = base_url().'a/agama/delete/'.encode($row['agama_id']).'/do';
         $data_nama = 'Agama '.$row['agama_nama'];
         ...
         $attrib_edit = array('class' => 'button',
             'style' => 'width: 50px; height: 20px; line-height: 20px; padding-left: 0px; padding-right: 0px;');
         $delete = base_url().'a/agama/delete/'.encode($row['agama_id']);
         $edit = base_url().'a/agama/edit/'.encode($row['agama_id']);
         $this->table->add_row('<td style="text-align: left;">'.$row['agama_nama'].'</td>',
            '<td style="text-align: left;">'.$row['agama_keterangan'].'</td>',
