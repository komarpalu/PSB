...
<div class="content-layout"><div class="content-layout-row"><div class="layout-cell layout-item-0" style="width: 100%" >
   <?php $template = array('table_open'         => '<table id="tsorter" class="tablesorter" style="width: 100%;">',
      ...
      if($button_add) echo '<p>'.anchor('a/kuota/add', 'Tambah', 'class = "button"').'</p>';
      $this->table->set_heading('TA', 'Gelombang', 'Jurusan', 'Kuota', 'Cadangan', 'Keterangan', 'Aksi');
      foreach ($query as $row){
         $url = base_url().'a/kuota/delete/'.encode($row['kuota_id']).'/do';
         $data_nama = 'Kuota '.$row['jur_nama'];
         ...
         $attrib_edit = array('class' => 'button',
            'style' => 'width: 50px; height: 20px; line-height: 20px; padding-left: 0px; padding-right: 0px;');
         $delete = base_url().'a/kuota/delete/'.encode($row['kuota_id']);
         $edit = base_url().'a/kuota/edit/'.encode($row['kuota_id']);
         $this->table->add_row('<td style="text-align: center;">'.$row['gel_ta'].'</td>',
            '<td style="text-align: left;">'.$row['gel_nama'].'</td>',
            '<td style="text-align: left;">'.$row['jur_nama'].'</td>',
            '<td style="text-align: center;">'.$row['kuota_jumlah'].'</td>',
            '<td style="text-align: center;">'.$row['kuota_cadangan'].'</td>',
            '<td style="text-align: left;">'.$row['kuota_keterangan'].'</td>',
